...
function generateCouponCode() {
    // Generate a random alphanumeric string with at least 3 'T' letters together between two numbers
    $randomString = generateRandomStringWithT(13); // Adjusted to make a total length of 13

    // Combine the parts to create the coupon code
    $coupon = $randomString;

    return $coupon;
}

function generateRandomStringWithT($length) {
    // Ensure at least 3 'T' letters together between two numbers
    $randomString = '';
    $tCount = 0;
    ...
    while (strlen($randomString) < $length) {
        ...
        if ($randomChar === 'T') {
            ...
            // Check if there are at least 3 'T' letters between two numbers
            if ($tCount >= 3) {
                $randomString .= 'TTT'; // Append at least 3 'T' letters together
                $tCount = 0;
            }
        } else {
            $tCount = 0;
...
// Generate 10 coupons
$generatedCoupons = generateCoupons(10);

// Display the generated coupons
                    ...
                    <div class="row">
                        <div class="col-md-8 d-flex flex-wrap">
                            ...
                            <div id="card_<?= $value['id']?>" class="card mb-3" style="max-width: 540px;">
                                <div class="row g-0">
                                    <div class="col-md-4">
                                        <img src="https://picsum.photos/id/63/400/400" class="img-fluid rounded-start" alt="...">
                                    </div>
                                    <div class="col-md-8">
                                        <div class="card-body">
                                            ...
                                            <div class="quantity-controls d-flex">
                                                <button class="quantity-decrease btn btn-warning" onclick="decreaseQuantity(<?= $value['id'] ?>)">-</button>
                                                <p class="card-text quantity"><span id="product_<?= $value['id'] ?>"><?= $value['quantity']?></span>  Adet </p>
                                                <input id="total_stock_<?=$value['id']?>" type="hidden" value="<?= $value['total_stock'] ?>">
                                                <button class="quantity-increase btn btn-success" onclick="increaseQuantity(<?= $value['id'] ?>)">+</button>
                                            </div>
                                            <p class="card-text">Birim fiyatı: <span id="price_<?= $value['id'] ?>"><?= $value['price'] ?></span> ₺</p>
                                            <p class="card-text">Toplam fiyat: <span id="total_price_<?= $value['id']?>"><?= $value['total_price']?></span> ₺</p>
                                            <button title="Ürünü Sil" onclick="removeProduct(<?= $value['id'] ?>)" class="float-right btn btn-danger"><i class="fa fa-trash"></i></button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-md-4">
                            <div class="offcanvas offcanvas-end show" tabindex="-1" id="offcanvas"
                                ...
                                <div class="offcanvas-body">
                                    <div class="row">
                                        <p>Sepet tutarı: <span id="total_price"><?= $totalPrice ?></span>₺</p>
                                    ...
                                    <div id="coupon_row" class="row">
                                        ...
                                        <button class="btn btn-primary" id="couponButton" type="button" onclick="checkCoupon()">Gir</button>
                                    </div>
                                    <div id="summary_row" class="row">
                                        <p>Toplam ödenecek tutar: <span id="summary"></span></p>
                                    </div>
                                    <div class="row">
                                        <button onclick="finalizeOrder()" class="btn btn-primary mt-2"> Siparişi Tamamla </button>
                                    </div>
        ...
        function summary(totalPrice, cargo_price = 0, discount = false){
            ...
            summary_element.text((total_price+cargo_price).toFixed(2))
                ...
                if (total_price > 3000){
                    ...
                    return {'description': '3000₺ Üzeri 1 Kilo kahve hediye ve ' ,'discount_amount': discountAmount, 'percantege': percantage}
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (res.status === "success") {
                            ...
                            var newItemP = $('<p>').text(discount.description + '%' + discount.percantege + ' İndirim! ').append($('<span>').css({'color': 'red', 'text-decoration': 'line-through'}).text(discount.discount_amount));
